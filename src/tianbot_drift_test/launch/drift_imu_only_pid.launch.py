...
def generate_launch_description():
    ...
    default_config = os.path.join(
        package_share, "config", "drift_imu_only_pid.yaml"
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "imu_yaw_sign",
                default_value="1.0",
                description="Right-handed Tianbot IMU uses +1.0 (CCW positive)",
            ),
            Node(
                package="tianbot_drift_test",
                executable="drift_imu_only_pid",
                name="drift_imu_only_pid",
                output="screen",
                parameters=[
                    config,
                    {
